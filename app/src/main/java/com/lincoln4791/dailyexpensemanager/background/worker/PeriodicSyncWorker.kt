...
/*                val database = Firebase.database.reference
                ...
                database.child(Constants.Common).child(Constants.BANNER).child(Constants.BANNERS).addListenerForSingleValueEvent(object:
                    ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        Log.d(SyncWorker.TAG,"Snapshot is -> $snapshot")
                        for(element in snapshot.children){
                            val data = element.getValue(Banner::class.java)
                            data!!.is_active = element.child("is_active").getValue(String::class.java)!!
                            Log.d(SyncWorker.TAG,"element is -> $data")
                            listOfBanners.add(data)
                        }

                        Log.d(SyncWorker.TAG,"Banner List is -> $listOfBanners")
